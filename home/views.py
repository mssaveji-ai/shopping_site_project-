...
class home_page(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        latest_products = Products.objects.filter(stock__gt=0,is_delete=False).order_by('-id')[:12]
        ...
        most_visited_products = Products.objects.filter(stock__gt=0, is_delete=False).annotate(visited_count=Count('visits')).order_by('-visited_count')[:6]
